...
//     Given multiple ways of doing a task,
//     which way will give the minimum or the maximum output.We can try to apply recursion.Once we get the recursive solution, we can go ahead to convert it to a dynamic programming one.

// Steps To Solve The Problem After Identification
//     Once the problem has been identified,
//     the following three steps comes handy in solving the problem :
